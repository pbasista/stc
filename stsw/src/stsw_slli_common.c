 ...
 * over a sliding window implemented using the SL implementation technique.
 *
 * @param
...
int stsw_slli_allocate (const int verbosity_level,
	...
	stsw->lr_size = sizeof (leaf_record_slli);
	/* we need to fill in the size of the branch record */
	stsw->br_size = sizeof (branch_record_slli);
	/*
	...
	/* it has no children in the beginning */
	stsw->tbranch[1].first_child = 0;
	/* it has no brother in the beginning */
	stsw->tbranch[1].branch_brother = 0;
...
int stsw_slli_get_leafs_depth (signed_integral_type leaf,
	...
	if (stsw->tleaf_first <= stsw->tleaf_last) {
		if ((stsw->tleaf_first <= leafs_number) &&
			...
			fprintf(stderr,	"stsw_slli_get_leafs_depth:\n"
...
int stsw_slli_edge_letter (signed_integral_type source_node,
	...
	if (source_node <= 0) {
		fprintf(stderr, "stsw_slli_edge_letter:\n"
	...
	if (target_node == 0) {
		...
		edge_letter_index = stsw_slli_get_leafs_sw_offset(
...
int stsw_slli_edge_depthscan (signed_integral_type child,
	...
	if (child == 0) {
		fprintf(stderr,	"stsw_slli_edge_depthscan:\n"
		 ...
		 */
		if (stsw_slli_get_leafs_depth(child, (size_t *)
					(&childs_depth), stsw) > 0) {
			fprintf(stderr,	"stsw_slli_edge_depthscan:\n"
					"Error: Could not get the depth "
					"of the child (%d). Exiting!\n",
					child);
			return (3);
		}
	}
...
int stsw_slli_edge_fastscan (signed_integral_type parent,
		...
		const text_file_sliding_window *tfsw,
		const suffix_tree_sliding_window_slli *stsw) {
	unsigned_integral_type edge_letter_index = 0;
	character_type edge_letter = 0;
	character_type text_letter = tfsw->text_window[position];
	/* FIXME: Try to remember the child's offset value for slowscan */
	if (child == 0) {
		/*
		...
		edge_letter_index = stsw->tbranch[child].head_position +
			stsw->tbranch[parent].depth;
		if (edge_letter_index > tfsw->total_window_size) {
			edge_letter_index -= (unsigned_integral_type)
	...
	edge_letter = tfsw->text_window[edge_letter_index];
 ...
 * 		against the first letter of a given edge
 * @param
 * last_match_position	The number of characters from the edge label
 * 			which already match the text
 ...
 * 			Moreover, the sign of this parameter
...
int stsw_slli_edge_slowscan (signed_integral_type parent,
	...
	if (child == 0) {
		fprintf(stderr,	"stsw_slli_edge_slowscan:\n"
				"Error: Invalid number of child (0)!\n");
		/*
	...
	if (tfsw->text_window[edge_letter_index] <
			tfsw->text_window[position]) {
		/*
		...
		/*
		 * tfsw->text_window[edge_letter_index] >
		 * tfsw->text_window[position]
		 *
...
int stsw_slli_quick_next_child (signed_integral_type *child,
	...
	if ((*child) > 0) {
		(*child) = stsw->tbranch[(*child)].branch_brother;
	} else {
		(*child) = stsw->tleaf[-(*child)].next_brother;
	}
	return (0);
}
...
int stsw_slli_edge_climb (signed_integral_type *parent,
	...
	if ((*parent) < 2) {
		fprintf(stderr,	"stsw_slli_edge_climb:\n"
 ...
 * 		In case of an error, a positive error number is returned.
 */
int stsw_slli_quick_edge_descend (signed_integral_type *parent,
		signed_integral_type *child,
		size_t *position,
		const text_file_sliding_window *tfsw,
		const suffix_tree_sliding_window_slli *stsw) {
	...
	/* we suppose that the parent node of this edge is a branching node */
	if ((*child) == 0) {
		fprintf(stderr,	"stsw_slli_quick_edge_descend:\n"
				"Error: Invalid number of child (0)!\n");
		...
		if (stsw_slli_get_leafs_depth((*child),
					&childs_depth, stsw) > 0) {
			fprintf(stderr,	"stsw_slli_quick_edge_descend:\n"
					"Error: Could not get the depth "
					"of the child (%d). Exiting!\n",
...
int stsw_slli_edge_descend (signed_integral_type *parent,
	...
	/* we suppose that the parent node of this edge is a branching node */
	if ((*child) == 0) {
		fprintf(stderr,	"stsw_slli_edge_descend:\n"
				"Error: Invalid number of child (0)!\n");
		return (1); /* invalid number of child */
	} else if ((*child) > 0) {
		(*position) = (*position) + stsw->tbranch[(*child)].depth -
...
int stsw_slli_quick_branch_once (signed_integral_type parent,
	...
	if (parent <= 0) {
		fprintf(stderr,	"stsw_slli_quick_branch_once:\n"
				"Error: Invalid number of parent (%d)!\n",
				parent);
	...
	if ((*child) == 0) {
		fprintf(stderr,	"stsw_slli_quick_branch_once:\n"
				"Warning: There is a branching node "
				"with no children!\n");
		return (2); /* branching failed (no children) */
	}
	while ((tmp = stsw_slli_edge_fastscan(parent, (*child), position,
					tfsw, stsw)) == (-1)) {
		/*
		 ...
		 */
		stsw_slli_quick_next_child(child, stsw);
	}
...
int stsw_slli_branch_once (signed_integral_type parent,
	...
	int tmp = 0; /* here we store the return value of the "fastscan" */
	if (parent <= 0) {
		fprintf(stderr,	"stsw_slli_branch_once:\n"
				"Error: Invalid number of parent (%d)!\n",
				parent);
		return (1); /* branching failed (invalid number of parent) */
	}
	...
	if ((*child) == 0) {
		fprintf(stderr,	"stsw_slli_branch_once:\n"
				"Warning: There is a branching node (%d) "
				"with no children!\n", parent);
		return (2); /* branching failed (no children) */
	}
	while ((tmp = stsw_slli_edge_fastscan(parent, (*child), position,
...
/**
 * A function which gets the number of child of the provided parent node,
 * which preceeds the child with the provided number.
 * For effectivity, this function does not check
 * whether the provided number of parent is valid.
 *
 * @param
 * parent	the branching node which is the parent of the child node
 * @param
 * desired_child	The child of the provided parent node,
 * 			for which we would like to get the number
 * 			of the child, which preceeds it
 * 			in the linked list of children of the parent node.
 * @param
 * child	When this function successfully returns, this variable
 * 		will be set to the the provided child
 * 		of the provided parent node.
 * 		If the provided parent node does not have such a child,
 * 		this variable will be set to zero.
 * @param
 * prev_child	When this function successfully returns, this variable
 * 		will be set to the the child of the provided parent node,
 * 		which preceeds the provided child node.
 * 		If the provided child node is the first child of the provided
 * 		parent node, this variable will be set to zero.
 * 		If the provided parent node does not have a child
 * 		with the number equal to the number of the desired_child,
 * 		this variable will be set to the number of the last child
 * 		of the parent node.
 * @param
 * stsw		the actual suffix tree
 *
 * @return	If we could successfully find the child preceeding
 * 		the provided child node of the provided parent node,
...
int stsw_slli_get_prev_child (signed_integral_type parent,
		...
		signed_integral_type *prev_child,
		const suffix_tree_sliding_window_slli *stsw) {
	/* we start by examining the first child of the provided parent node */
	(*child) = stsw->tbranch[parent].first_child;
	if ((*child) == 0) {
		fprintf(stderr,	"stsw_slli_get_prev_child:\n"
				"Error: The provided branching node (%d) "
				"does not have any children!\n", parent);
		return (1); /* branching failed (no children) */
	}
	(*prev_child) = 0;
	/*
	 * while the currently examined tmp_child
	 * differs from the provided child
	 * and while the prev_child is not the last child
	 * of the provided parent node
	 */
	while (((*child) != 0) && ((*child) != desired_child)) {
		/*
		 * we advance to the next child
		 * of the provided parent node
		 */
		stsw_slli_next_child(child, prev_child, stsw);
	}
...
int stsw_slli_go_down (signed_integral_type grandpa,
	...
	if (stsw->tbranch[(*parent)].depth == target_depth) {
		...
		return (0);
			...
			if (grandpa == 0) {
				fprintf(stderr, "SL10: (*active_node) == 0\n");
				abort();
			}
	while (stsw_slli_quick_branch_once((*parent), &child, (*position),
				tfsw, stsw) == 0) {
		if ((tmp = stsw_slli_edge_depthscan(child, target_depth,
						stsw)) == -1) {
			/*
			 ...
			 */
			grandpa = (*parent);
			stsw_slli_quick_edge_descend(parent, &child,
					position, tfsw, stsw);
			if ((*parent) < 0) { /* parent is a leaf */
				fprintf(stderr,	"stsw_slli_go_down:\n"
...
int stsw_slli_go_up (signed_integral_type *parent,
	...
	if ((*child) == 0) { /* child number is invalid */
		fprintf(stderr,	"stsw_slli_go_up:\n"
				"Error: Invalid number of child (0)!\n");
		return (1); /* invalid number of child */
	} else if ((*child) > 0) { /* child is a branching node */
	...
	} while (stsw_slli_edge_climb(parent, child, stsw) == 0);
	/*
...
int stsw_slli_create_leaf (signed_integral_type parent,
		...
		suffix_tree_sliding_window_slli *stsw) {
	if (parent <= 0) {
		fprintf(stderr,	"stsw_slli_create_leaf:\n"
	...
	if (prev_child == 0) {
		stsw->tbranch[parent].first_child = new_leaf;
	} else if (prev_child > 0) {
		stsw->tbranch[prev_child].branch_brother = new_leaf;
	} else { /* prev_child < 0 */
		stsw->tleaf[-prev_child].next_brother = new_leaf;
	}
	/* we set the parent and the next brother of the new leaf node */
	stsw->tleaf[-new_leaf].parent = parent;
	stsw->tleaf[-new_leaf].next_brother = child;
	return (0);
}
...
int stsw_slli_split_edge (signed_integral_type *parent,
	...
	int child_first = 0;
	if ((*parent) <= 0) {
		fprintf(stderr,	"stsw_slli_split_edge:\n"
				"Error: Invalid number of parent (%d)!\n",
				(*parent));
		return (1); /* invalid number of parent */
	}
	 ...
	 * the mismatching character in the sliding window was "larger"
	...
	if ((*prev_child) == 0) {
		stsw->tbranch[(*parent)].first_child = new_branching_node;
	} else if ((*prev_child) > 0) {
		stsw->tbranch[(*prev_child)].branch_brother =
			new_branching_node;
	} else { /* (*prev_child) < 0 */
		stsw->tleaf[-(*prev_child)].next_brother = new_branching_node;
	}
	/*
	...
	stsw->tbranch[new_branching_node].first_child = (*child);
	/* the following value will be set right away */
	stsw->tbranch[new_branching_node].branch_brother = 0;
	 ...
	 */
	if ((*child) > 0) {
		stsw->tbranch[new_branching_node].branch_brother =
			stsw->tbranch[(*child)].branch_brother;
...
int stsw_slli_simple_traverse_from (FILE *stream,
	...
	child = stsw->tbranch[starting_node].first_child;
		...
		if (child > 0) {
			...
			if (childs_parent != starting_node) {
				fprintf(stderr,	"Error: Something went wrong."
						"\nChild's parent as stored "
						"in the child node (%d)\n"
						"is not the same as "
						"the actual parent (%d).\n",
						childs_parent, starting_node);
			...
			stsw_print_edge(stream, 0,
					...
					childs_offset, tfsw);
			stsw_slli_simple_traverse_from(stream, child,
			...
			if (stsw_slli_get_leafs_depth(child, (size_t *)
						(&childs_depth), stsw) > 0) {
				fprintf(stderr,	"stsw_slli_simple_"
						"traverse_from:\n"
						"Error: Could not get "
			...
			childs_offset = stsw_slli_get_leafs_sw_offset(child,
					tfsw, stsw);
	...
	while (child != 0) {
		if (child > 0) {
			...
			stsw_print_edge(stream, 0, starting_node, child,
					...
					childs_depth, log10bn, log10l,
					childs_offset, tfsw);
			stsw_slli_traverse_from(stream, child, log10bn, log10l,
					tfsw, stsw);
			/*
			 * there would be some overhead if we called
			 * the next child function, so we do it simply
...
int stsw_slli_traverse (const int verbosity_level,
	...
	if (traversal_type == tt_detailed) {
		...
		if (stsw_slli_traverse_from(stream, start_node,
					log10bn, log10l,
					tfsw, stsw) > 0) {
		...
		if (stsw_slli_simple_traverse_from(stream, start_node,
					log10bn, log10l,
					tfsw, stsw) > 0) {
			fprintf(stderr, "Error: The traversal "
					"from the branching node\n"
...
int stsw_slli_delete (const int verbosity_level,
		suffix_tree_sliding_window_slli *stsw) {
	if ((stsw->tleaf == NULL) && (stsw->tbranch == NULL)) {
	...
	stsw->tleaf_last = 0;
	/*
	 * The other entries of the suffix_tree_sliding_window_slli struct
