 ...
 */
int stsw_shti_simulate_suffix_link_top_down (signed_integral_type grandpa,
	...
	if (grandpa <= 0) { /* grandpa is either a leaf or undefined */
		fprintf(stderr,	"stsw_shti_simulate_suffix_link_top_down:\n"
	...
	if ((retval = stsw_shti_go_down(grandpa, parent, target_depth,
 ...
 * 		If an error occurs, a positive error number is returned.
 */
int stsw_shti_simulate_suffix_link_bottom_up (signed_integral_type *parent,
	...
	if (child == 0) { /* child number is undefined */
		fprintf(stderr,	"stsw_shti_simulate_suffix_link_bottom_up:\n"
	...
	(*parent) = 0; /* we invalidate the parent's node */
	/* if we were able to locate the desired depth */
	if ((retval = stsw_shti_go_up(parent, &child, target_depth,
					stsw)) == 0) {
		/* we establish a new suffix link */
		stsw->tbranch[starting_from].suffix_link = (*parent);
		return (0);
 ...
 * 		If an error occurs, a positive error number is returned.
 */
int stsw_shti_ukkonen_prolong_suffix (const int variation,
		size_t starting_position,
	 ...
	 */
	while ((tmp = stsw_shti_branch_once((*active_node), &child,
					position, tfsw, stsw)) == 0) {
		/*
		 * while there is a candidate branching edge,
		 * we check it for a complete match
		 */
		if ((slowscan_retval = stsw_shti_edge_slowscan((*active_node),
			 ...
			 */
			stsw_shti_quick_edge_descend(active_node, &child,
					&position, tfsw, stsw);
			if ((*active_node) < 0) {
				/*
			...
			if (stsw_shti_split_edge(active_node, letter, &child,
			...
			if (stsw_shti_create_leaf((*active_node), letter,
						-new_leaf, tfsw, stsw) > 0) {
			...
			if (variation == 1) {
				 ...
				 */
				if ((sls_retval =
				stsw_shti_simulate_suffix_link_bottom_up(
							active_node,
							latest_child,
							(*sl_targets_depth),
							stsw)) == 0) {
				...
				/* if it has encountered an error */
					...
					fprintf(stderr,
					"stsw_shti_ukkonen_prolong_suffix:\n"
				 ...
				 */
				if ((sls_retval =
				stsw_shti_simulate_suffix_link_top_down(
							grandpa, active_node,
							(*sl_targets_depth),
	...
	if (tmp == 1) {
		...
		if (stsw_shti_create_leaf((*active_node), letter, -new_leaf,
					tfsw, stsw)) {
			fprintf(stderr, "Error: Could not create "
					"the new leaf edge "
#ifdef	SUFFIX_TREE_TEXT_WIDE_CHAR
		...
		if (tfsw->elm_method == 1) { /* batch update by M. Senft */
			 ...
			 */
			if ((*active_node) != 1) {
				if (stsw_shti_send_credit((*active_node),
 ...
 */
int stsw_shti_ukkonen_prolong_suffixes (const int variation,
		size_t *starting_position,
		size_t ending_position,
		size_t *active_index,
		signed_integral_type *active_node,
	...
	if ((tmp = stsw_shti_ukkonen_prolong_suffix(variation,
					(*starting_position),
		 ...
		 * of the function stsw_shti_ukkonen_prolong_suffix
		 */
		++(*starting_position);
	 ...
	 * the call to the stsw_shti_ukkonen_prolong_suffix
	 */
	while (tmp == 0) {
		++(*starting_position);
		if ((*starting_position) > tfsw->total_window_size) {
		...
		tmp = stsw_shti_ukkonen_prolong_suffix(variation,
...
int stsw_shti_create_ukkonen (FILE *stream,
	...
	/* the minimum number of edges observed */
	size_t min_edges = 0;
	/*
	 ...
	 * of edges observed.
	 */
	size_t min_edges_ap_bti = 0;
	/* the average number of edges observed */
	double avg_edges = 0;
	/* the maximum number of edges observed */
	size_t max_edges = 0;
	/*
	 * the text index of the beginning of the active part
	 * of the sliding window (or ap_bti),
...
/* POSIX threads-related variables */
	...
	if (variation == 0) {
		printf("Creating and maintaining the suffix tree "
				...
				"using the SH implementation technique "
		...
		fprintf(stderr, "stsw_shti_create_ukkonen:\n"
	...
	fprintf(stderr, "Warning:\n"
			"========\n"
			"You have requested to use the SH implementation "
			"technique.\nPlease, be aware that it is very slow "
			"with finding the last child\nof a single parent. "
			"Since this is essential when maintaining\n"
			"the suffix tree over a sliding window, you should "
			"expect\na particularly slower program execution\n"
			"than whith the SL implementation technique.\n\n");
#ifdef	SUFFIX_TREE_TEXT_WIDE_CHAR
	fprintf(stderr, "Warning:\n"
			"========\n"
			"This program is compiled "
			"with the support for Unicode characters\n"
			"and you have chosen a hash table based "
			"implementation technique (SH).\n"
			"This combination is not recommended,\n"
			"because it will most likely result\n"
			"in hanging the program during the lookup\n"
			"of the last child of some parent node.\n\n");
#endif
	if (stsw_shti_allocate(verbosity_level, tfsw, stsw) > 0) {
		fprintf(stderr, "stsw_shti_create_ukkonen:\n"
	 ...
	 * during the suffix tree construction and maintenance.
	 *
	 * The previous call to stsw_shti_allocate has already ensured
	 * that the size of the hash table for the edges is large enough
	 * to hold all the edges that the sliding window can possibly have,
	 * provided that there will not be many hashing collisions.
	 * That's why we do not reallocate this hash table here.
	 * The hash table will be reallocated and rehashed only if necessary,
	 * in the case when we experience a hashing collision,
	 * which can not be resolved otherwise.
	 */
	if (stsw_shti_reallocate(verbosity_level, tfsw->sw_block_size *
				tfsw->ap_scale_factor - 1, (size_t)(0),
				tfsw, stsw) > 0) {
		fprintf(stderr, "stsw_shti_create_ukkonen:\n"
	...
	while (++block_to_process < tfsw->ap_scale_factor) {
		...
		for (; i <= ending_position; ++i) {
			 ...
			 */
			++tfsw->ap_window_size;
			if (stsw_shti_ukkonen_prolong_suffixes(variation,
						&starting_position, i,
	...
	if (benchmark == 2) {
		 ...
		 */
		if (stsw_shti_traverse(verbosity_level, stream, traversal_type,
					tfsw, stsw) != 0) {
	...
	if (verbosity_level > 1) {
		...
		min_edges = stsw->edges;
		min_edges_ap_bti = ap_window_begin_text_idx;
		avg_edges = (double)(stsw->edges);
		max_edges = stsw->edges;
		max_edges_ap_bti = ap_window_begin_text_idx;
	...
	while (1) {
		...
		for (; i < ending_position; ++i) {
			...
			if (stsw_shti_delete_longest_suffix(&starting_position,
						&active_index, &active_node,
						tfsw, stsw) > 0) {
			...
			if (verbosity_level > 1) {
				 ...
				 */
				if (stsw->edges < min_edges) {
					min_edges = stsw->edges;
					min_edges_ap_bti =
						ap_window_begin_text_idx;
				}
				avg_edges += (double)(stsw->edges);
				if (stsw->edges > max_edges) {
					max_edges = stsw->edges;
					max_edges_ap_bti =
						ap_window_begin_text_idx;
				}
		...
		if (benchmark == 2) {
			 ...
			 */
			if (stsw_shti_traverse(verbosity_level, stream,
						traversal_type,
						tfsw, stsw) != 0) {
		...
		if ((tfsw->elm_method == 1) &&
			...
			if (stsw_shti_edge_labels_batch_update(tfsw,
						stsw) > 0) {
...
thread_joining:
	...
	while (1) {
		 ...
		 * stsw_shti_delete_longest_suffix
		 * and stsw_shti_ukkonen_prolong_suffixes.
		 */
		for (; i < ending_position; ++i) {
			/* at first, we have to delete the longest suffix */
			if (stsw_shti_delete_longest_suffix(&starting_position,
...
final_part:
	...
	stsw_print_stats(verbosity_level, tfsw->characters_read,
			 ...
			 * memory used
			 *
			 * We do not keep track of the maximum portion
			 * of the additional memory used by the hash
			 * settings. That's why we supply the total
			 * size of this memory here.
			 */
			stsw->hs->allocated_size +
