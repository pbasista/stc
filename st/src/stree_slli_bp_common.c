...
/**
 * @file
 * SLLI_BP common functions implementation.
...
int st_slli_bp_allocate (size_t length,
	...
	stree->lr_size =  sizeof (leaf_record_slli_bp);
	...
	stree->br_size =  sizeof (branch_record_slli_bp);
	 ...
	 *
	 * stree->tbranch[1] is the root and therefore has no parent
	 * (it can not have any)
	 */
...
int st_slli_bp_quick_branch_once (signed_integral_type parent,
	...
	while ((tmp = st_slli_bp_edge_fastscan(parent, (*child), position,
					text, stree)) == (-1)) {
		/*
		 ...
		 */
		st_slli_bp_quick_next_child(child, stree);
	}
...
int st_slli_bp_branch_once (signed_integral_type parent,
	...
	while ((tmp = st_slli_bp_edge_fastscan(parent, (*child), position,
		 ...
		 * which label starts with the desired character.
		 */
		st_slli_bp_next_child(child, prev_child, stree);
	}
	if (tmp == 0) { /* fastscan succeeded */
		return (0); /* branching succeeded (matching edge found) */
	} else {
...
int st_slli_bp_go_up (signed_integral_type *parent,
	...
	} while (st_slli_bp_edge_climb(parent, child, stree) == 0);
	/*
 ...
 * 		In case of an error, a positive error number is returned.
 */
int st_slli_bp_simple_traverse_from (FILE *stream,
	...
	while (child != 0) {
		if (child > 0) {
			...
			st_print_edge(stream, 0,
					...
					text);
			st_slli_bp_simple_traverse_from(stream, child,
					log10bn, log10l,
 ...
 * 		In case of an error, a positive error number is returned.
 */
int st_slli_bp_traverse_from (FILE *stream,
		signed_integral_type starting_node,
		size_t log10bn,
		size_t log10l,
		const char *internal_text_encoding,
	...
	while (child != 0) {
		if (child > 0) {
			...
			st_print_edge(stream, 0, starting_node, child,
					...
					childs_offset, internal_text_encoding,
					text);
			st_slli_bp_traverse_from(stream, child, log10bn, log10l,
					internal_text_encoding, text, length,
					stree);
			/*
			 * there would be some overhead if we called
...
int st_slli_bp_traverse (FILE *stream,
	...
	if (traversal_type == tt_detailed) {
		...
		if (st_slli_bp_traverse_from(stream, start_node,
					log10bn, log10l,
		...
		if (st_slli_bp_simple_traverse_from(stream, start_node,
					log10bn, log10l,
					internal_text_encoding,
					text, length, stree) > 0) {
			fprintf(stderr, "Error: The traversal "
