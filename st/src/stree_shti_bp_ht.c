...
/**
 * @file
 * SHTI_BP hashing-related functions implementation.
...
int stree_shti_bp_er_letter (edge_record er,
		...
		const suffix_tree_shti_bp *stree) {
	if (er.source_node <= 0) {
		fprintf(stderr, "stree_shti_bp_er_letter:\n"
				"Error: The provided edge_record\n"
				"contains a source node (%d), "
				"which is not a branching node.\n",
				er.source_node);
...
int stree_shti_bp_edge_letter (signed_integral_type source_node,
		...
		const suffix_tree_shti_bp *stree) {
	if (source_node <= 0) {
		fprintf(stderr, "stree_shti_bp_edge_letter:\n"
	...
	do {
		...
		for (i = 0; i < original_tedge_size; ++i) {
			...
			if (er_empty(original_tedge[i]) == 0) {
				...
				if (stree_shti_bp_edge_letter(source_node,
							&letter, target_node,
							text, stree) > 0) {
				 ...
				 */
				if (stree_shti_bp_ht_insert(source_node,
							letter, target_node,
							0, text, stree) != 0) {
 ...
 */
int stree_shti_bp_cuckoo_ht_insert (size_t call_depth,
	...
	if (stree_shti_bp_er_letter(stree->tedge[idx],
	...
	stree->tedge[idx].source_node = current_source_node;
	stree->tedge[idx].target_node = current_target_node;
	if (stree_shti_bp_cuckoo_ht_insert(call_depth, original_source_node,
		...
		do {
			...
			for (i = 0; i < chf_number; ++i) {
				...
				if (er_empty(stree->tedge[idx]) == 0) {
					 ...
					 * this edge record
					 */
					if (stree_shti_bp_er_key_matches(
						source_node,
			...
			new_source_node = stree->tedge[idx].source_node;
			new_target_node = stree->tedge[idx].target_node;
			if (stree_shti_bp_edge_letter(new_source_node,
			...
			stree->tedge[idx].source_node = source_node;
			stree->tedge[idx].target_node = target_node;
			if (stree_shti_bp_cuckoo_ht_insert((size_t)(0),
						source_node, letter,
						new_source_node,
						new_letter, new_target_node,
			...
			if (rehash_allowed != 0) {
				...
				if (stree_shti_bp_ht_rehash(&new_tedge_size,
							text, stree) == 0) {
					/*
					 * we adjust the size
...
finish:		return (0);
		...
		if (er_empty(stree->tedge[i]) == 0) {
			if (stree_shti_bp_er_key_matches(source_node, letter,
			...
			while ((er_empty(stree->tedge[i]) == 0) &&
					i != first_i) {
				if (stree_shti_bp_er_key_matches(source_node,
