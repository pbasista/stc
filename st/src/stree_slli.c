 ...
 * using the implementation type SLLI.
 */
#include "stree_slli.h"
 ...
 */
int st_slli_simulate_suffix_link_top_down (signed_integral_type grandpa,
	...
	if ((retval = st_slli_go_down(grandpa, parent, target_depth, &position,
 ...
 */
int st_slli_simple_insert_suffix (size_t starting_position,
		const character_type *text,
		size_t length,
		suffix_tree_slli *stree) {
	...
	if ((tbranch_size == stree->branching_nodes) &&
		...
		if (st_slli_reallocate((size_t)(0), length, stree) > 0) {
			fprintf(stderr,	"Error: Could not "
					"reallocate memory!\n");
			return (1);
		}
	}
	while ((tmp = st_slli_branch_once(parent, &child, &prev_child,
					position, text, stree)) == 0) {
		/*
		 * while there is a candidate branching edge,
		 * we check it for a complete match
		 */
		if (st_slli_edge_slowscan(parent, child, position,
			 ...
			 */
			st_slli_edge_descend(&parent, &child, &prev_child,
			...
			st_slli_split_edge(&parent, &child, &prev_child,
					&position, last_match_position,
				(unsigned_integral_type)(starting_position),
					stree);
			st_slli_create_leaf(parent, child, prev_child,
 ...
 * 		If an error occurs, a nonzero error number is returned.
 */
int st_slli_mccreight_insert_suffix (signed_integral_type *parent,
	...
	while ((tmp = st_slli_branch_once((*parent), &child, &prev_child,
		...
		if (st_slli_edge_slowscan((*parent), child, position,
			...
			if (st_slli_simulate_suffix_link_top_down(grandpa,
 ...
 * 		If an error occurs, a positive error number is returned.
 */
int st_slli_simple_prolong_suffix (size_t starting_position,
	...
	while ((tmp = st_slli_branch_once(parent, &child, &prev_child,
		 ...
		 */
		if ((slowscan_return = st_slli_edge_slowscan(parent, child,
 ...
 * 		If an error occurs, a positive error number is returned.
 */
int st_slli_ukkonen_prolong_suffix (size_t starting_position,
		size_t ending_position,
		size_t *active_index,
		signed_integral_type *parent,
		signed_integral_type *sl_source,
 ...
 * 		If an error occurs, a nonzero error number is returned.
 */
int st_slli_simple_prolong_suffixes (size_t *starting_position,
		size_t ending_position,
		const character_type *text,
		size_t length,
		suffix_tree_slli *stree) {
	...
	while ((tmp = st_slli_simple_prolong_suffix(i, ending_position, text,
					length, stree)) == (-1)) {
		++i;
	}
	...
	while (tmp == 0) {
		++i;
		tmp = st_slli_simple_prolong_suffix(i, ending_position, text,
					length, stree);
	}
 ...
 * 		If an error occurs, a nonzero error number is returned.
 */
int st_slli_ukkonen_prolong_suffixes (size_t *starting_position,
		size_t ending_position,
		size_t *active_index,
	...
	if ((tbranch_size < maximum_possible_size) &&
			(tbranch_size < length)) {
		if (st_slli_reallocate(maximum_possible_size,
					length, stree) > 0) {
			fprintf(stderr,	"Error: Could not "
					"reallocate memory!\n");
			return (1);
	...
	while ((tmp = st_slli_ukkonen_prolong_suffix((*starting_position),
	...
	while (tmp == 0) {
		++(*starting_position);
		tmp = st_slli_ukkonen_prolong_suffix((*starting_position),
...
int st_slli_create_simple_mccreight (const character_type *text,
	...
	printf("Creating suffix tree using simple McCreight's "
		"style algorithm\n\n");
	if (st_slli_allocate(length, stree) > 0) {
		fprintf(stderr,	"Allocation error. Exiting.\n");
		return (1);
	}
	for (i = 1; i < length + 2; ++i) {
		if (st_slli_simple_insert_suffix(i, text, length, stree) > 0) {
...
int st_slli_create_mccreight (const character_type *text,
	...
	for (i = 1; i < length + 2; ++i) {
		if (st_slli_mccreight_insert_suffix(&starting_node, &sl_source,
					&sl_targets_depth, i, text,
					length, stree) > 0) {
			fprintf(stderr,	"Could not insert suffix number "
					"%zu. Exiting.\n", i);
			return (2);
...
int st_slli_create_simple_ukkonen (const character_type *text,
	...
	for (i = 2; i <= length + 2; ++i) {
		if (st_slli_simple_prolong_suffixes(&starting_position, i,
					text, length, stree) != 0) {
...
int st_slli_create_ukkonen (const character_type *text,
	 ...
	 */
	for (i = 2; i <= length + 2; ++i) {
		if (st_slli_ukkonen_prolong_suffixes(&starting_position, i,
