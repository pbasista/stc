...
int st_shti_allocate (size_t length,
	...
	stree->br_size =  sizeof (branch_record_shti);
	 ...
	 *
	 * stree->tbranch[1] is the root and it has
	 * the depth of zero (which never changes)
	 */
...
int st_shti_reallocate (size_t desired_tbranch_size,
	...
	if (desired_tedge_size > 0) {
		 ...
		 * stree_shti_ht_rehash. This function also updates
		 * the tedge_size entry of the suffix tree struct.
		 */
		if (stree_shti_ht_rehash(&new_tedge_size, text, stree) == 0) {
...
int st_shti_quick_next_child (signed_integral_type parent,
	...
	if ((*child) == 0) {
		 ...
		 */
		if (stree_shti_ht_lookup(parent, letter, child,
					text, stree) == 0) {
	...
	do {
		++letter;
		retval = stree_shti_ht_lookup(parent, letter, child,
				text, stree);
 ...
 * It is a quick variant of the function,
 * which means that it does not keep track of previous child.
 *
 * @param
 * parent	the node from which the given edge starts
 ...
 * @param
 * position	The position in the text of the beginning
 * 		of the substring which matches the letters
 * 		of the given edge. When this function returns,
...
int st_shti_branch_once (signed_integral_type parent,
	...
	if (stree_shti_ht_lookup(parent, letter, child, text, stree) == 0) {
...
int st_shti_go_down (signed_integral_type grandpa,
	...
	/* we suppose that the grandpa is a branching node */
	(*parent) = grandpa;
	...
	while (st_shti_branch_once((*parent), &child, (*position),
				text, stree) == 0) {
		if ((tmp = st_shti_edge_depthscan(child, target_depth,
						ef_length, stree)) == (-1)) {
			/*
			 ...
			 */
			grandpa = (*parent);
			st_shti_quick_edge_descend(parent, &child,
			 ...
			 * by the previous calls of the edge_descend function.
			 */
			(*parent) = child;
			return (0);
...
int st_shti_create_leaf (signed_integral_type parent,
	...
	if (stree_shti_ht_insert(parent, letter, new_leaf, 1, text, stree)
			!= 0) {
...
int st_shti_split_edge (signed_integral_type *parent,
	...
	if (stree_shti_ht_insert((*parent), letter, new_branching_node,
				1, text, stree) != 0) {
	 ...
	 */
	if (stree_shti_ht_insert(new_branching_node, letter,
 ...
 * 		In case of an error, a positive error number is returned.
 */
int st_shti_simple_traverse_from (FILE *stream,
	...
	if (st_shti_quick_next_child(starting_node, &child, text, stree)
			!= 0) {
	...
	do {
		if (child > 0) {
			...
			st_print_edge(stream, 0,
					...
					text);
			st_shti_simple_traverse_from(stream, child,
					log10bn, log10l,
		 ...
		 */
	} while (st_shti_quick_next_child(starting_node, &child, text, stree)
			== 0);
	return (0);
}
 ...
 * 		In case of an error, a positive error number is returned.
 */
int st_shti_traverse_from (FILE *stream,
		signed_integral_type starting_node,
		size_t log10bn,
		size_t log10l,
		const char *internal_text_encoding,
	...
	do {
		if (child > 0) {
			...
			st_print_edge(stream, 0, starting_node, child,
					...
					childs_offset, internal_text_encoding,
					text);
			st_shti_traverse_from(stream, child, log10bn, log10l,
					internal_text_encoding, text, length,
					stree);
		} else { /* child < 0 */
			childs_depth = (unsigned_integral_type)((length + 2)
...
int st_shti_traverse (FILE *stream,
	...
	if (traversal_type == tt_detailed) {
		...
		if (st_shti_traverse_from(stream, start_node,
					log10bn, log10l,
		...
		if (st_shti_simple_traverse_from(stream, start_node,
					log10bn, log10l,
					internal_text_encoding,
					text, length, stree) > 0) {
			fprintf(stderr, "Error: The traversal "
...
int st_shti_delete (suffix_tree_shti *stree) {
	...
	stree->tedge_size = 0;
	/*
	 * The other entries of the suffix_tree_shti struct
